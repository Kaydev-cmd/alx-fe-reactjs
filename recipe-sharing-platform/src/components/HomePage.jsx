...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    fetch("/data.json")
      .then((res) => res.json())
      .then((data) => setRecipes([...data, ...localRecipes]))
      .catch((err) => console.error("Error loading recipes", err));
  }, []);

  return (
    <>
      <div className="container flex justify-end" style={{ margin: "16px 0" }}>
        <button className="bg-blue-600" onClick={() => navigate("/add-recipe")}>
          Add Recipe
        </button>
      </div>
